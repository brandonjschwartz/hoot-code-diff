...
( function( api ) {
	...
	// Extends our custom "hootubix-premium" section.
...
jQuery(document).ready(function($) {
	...
	var $premiumhead = $('#accordion-section-premium > h3', 'body:not(.hootubix-bcomp)');
	if ( $premiumhead.length ) {
		$premiumhead.prepend('<i class="fa fa-star"></i> ');
		$premiumhead.on( 'click', function(){
			$('body').addClass('hootubix-display-premiumuse');
		});
		...
		$('.customize-section-back').on('click', function(){
			$('body').removeClass('hootubix-display-premiumuse');
		});
	}
