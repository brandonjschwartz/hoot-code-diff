...
function hootubix_theme_customizer_options() {
	...
	$settings['logo_image'] = array(
		...
		'section'         => $section,
		'type'            => 'image',
		'priority'        => '185', // Replaced by WP's custom_logo if available // Update in premium if needed
	...
	$settings['logo_image_width'] = array(
		...
		'section'         => $section,
		'type'            => 'text',
		'priority'        => '186', // Keep it with image logo // Update in premium if needed
		'default'         => 200,
	...
	$sections[ $section ] = array(
		...
		'description' => __('Control various color options in the premium version for fonts, backgrounds, contrast, highlight, accent etc.', 'hoot-ubix'),
	);
	...
	$settings['wt_html_slider'] = array(
		'label'       => __( 'Slides', 'hoot-ubix' ),
		'section'     => $section,
		'type'        => 'content',
		'priority'    => 735, // Non static options must have a priority
		'content'     => __( 'Premium version of this theme comes with capability to create <strong>Unlimited slides</strong>.', 'hoot-ubix' ),
	);
	...
	$hootubix_customizer_load_premiumsection = apply_filters( 'hootubix_customize_load_premiumsection', file_exists( $premium_features_file ) );
	if ( $hootubix_customizer_load_premiumsection ) :

		$section = 'premium';

		$sections[ $section ] = array(
			'title'       => __( 'Premium Options', 'hoot-ubix' ),
			'priority'    => 46,
		);

		include( HYBRIDEXTEND_INC . 'admin/premium.php' );
		$pcontent = '';
		$plink = ( !empty( $hootubix_cta_url ) ) ? $hootubix_cta_url : '';
		$pdemo = ( !empty( $hootubix_cta_demo_url ) ) ? $hootubix_cta_demo_url : '';
		...
		if ( !empty( $hootubix_options_premium ) && is_array( $hootubix_options_premium ) ):
			$pcontent .= '<div id="hootubix-psection" class="hootubix-psection">';
				$pcontent .= '<h1 class="centered">' . $hootubix_intro['name'] . '</h1>';
				$pcontent .= '<p class="hootubix-upsell-intro centered">' . $hootubix_intro['desc'] . '</p>';
				$pcontent .= '<p class="hootubix-upsell-cta centered pcontent-actions pcontent-top">';
					if ( !empty( $plink ) ) $pcontent .= '<a class="button button-primary" href="' . esc_url( $plink ) . '" target="_blank"><i class="fa fa-star"></i> ' . __( 'Buy Now', 'hoot-ubix' ) . '</a>';
					if ( !empty( $pdemo ) ) $pcontent .= '<a class="button button-secondary" href="' . esc_url( $pdemo ) . '" target="_blank"><i class="fa fa-eye"></i> ' . __( 'Theme Demo', 'hoot-ubix' ) . '</a>';
				$pcontent .= '</p>';
				$pcontent .= '<div class="hootubix-psection-content">';
					foreach ( $hootubix_options_premium as $pofeature ) :
					...
					endforeach;
				$pcontent .= '</div>';
				$pcontent .= '<p class="hootubix-upsell-cta centered pcontent-actions pcontent-bottom">';
					if ( !empty( $plink ) ) $pcontent .= '<a class="button button-primary" href="' . esc_url( $plink ) . '" target="_blank"><i class="fa fa-star"></i> ' . __( 'Buy Now', 'hoot-ubix' ) . '</a>';
					if ( !empty( $pdemo ) ) $pcontent .= '<a class="button button-secondary" href="' . esc_url( $pdemo ) . '" target="_blank"><i class="fa fa-eye"></i> ' . __( 'Theme Demo', 'hoot-ubix' ) . '</a>';
				$pcontent .= '</p>';
			$pcontent .= '</div>';
		...
		if ( !empty( $pcontent ) )
			$settings['premium-use'] = array(
				// 'label'       => __( 'Premium Options', 'hoot-ubix' ),
				'section'     => $section,
				'type'        => 'content',
...
function hootubix_customizer_modify_default_options( $wp_customize ) {
	...
	if ( function_exists( 'the_custom_logo' ) ) {
		...
		$wp_customize->get_control( 'custom_logo' )->priority = 185; // Replaces theme's logo_image // Update in premium if needed
