		...
		if ( is_array( $sections ) && !empty( $sections ) ) :
			foreach ( $sections as $key => $section ) :
				if ( empty( $section[ 'sortitem_hide' ] ) ):
					...
					$key = apply_filters( 'hootubix_frontpage_sections_switch', $key );
						...
						case 'content':
							...
							<div id="frontpage-page-content" <?php hybridextend_attr( 'frontpage-area', $key, 'frontpage-area ' . esc_attr( $background_class ) ); ?>>
								...
								if( is_home() ) :
									...
									?><div class="hgrid hootubix-blogposts main-content-grid"><?php

										if ( !empty( $section['title'] ) )
											echo '<div class="hgrid-span-12"><h3 class="hootubix-blogposts-title">' . wp_kses_post( $section['title'] ) . '</h3></div>';
